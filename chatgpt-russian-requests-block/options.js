document.addEventListener("DOMContentLoaded", async () => {
  ...
  const loadSettings = async () => {
    try {
      ...
      domainsEl.value = (
        result.blockedDomains || ["chat.openai.com", "chatgpt.com"]
      ).join("\n");
  ...
  const saveSettings = async () => {
    try {
      ...
      if (domains.length === 0)
        throw new Error("Список доменов не может быть пустым");
      if (regions.length === 0)
        throw new Error("Список регионов не может быть пустым");
      ...
      showStatus(
        `❌ ${error.message || "Ошибка при сохранении настроек"}`,
        false,
      );
    }
  };
